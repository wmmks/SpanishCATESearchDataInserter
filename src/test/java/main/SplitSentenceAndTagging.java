...
public class SplitSentenceAndTagging {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        for (int articleID = FoldName.ARTICLE_ID_START; articleID <= FoldName.ARTICLE_ID_END; articleID++) {
            ...
            try {
                readFileController = new ReadFileController(inputFileName + articleID);
            } catch (Exception e) {
            ...
            for (String s : sentenceList) {
                article.append(treeTagger.getPOSTag(s));
                article.append(FoldName.SENTENCE_FLAG);
            }
            // 依據要求選擇 CORRECT 或 ORIGINAL
            writeFileController.writeDataIntoFile(outputFileName + articleID, article.toString());
        }
    }
